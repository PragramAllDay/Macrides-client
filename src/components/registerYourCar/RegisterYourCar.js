...
export default function RegisterYourCar({ handleClose }) {
  ...
  const [trackerStatus, setTrackerStatus] = React.useState(true);
  const [insuranceStatus, setInsuranceStatus] = React.useState(true);
  ...
  const validate = (values) => {
    ...
    if (!values.regCity) {
      errors.regCity = "Registration City is required";
    }
    if (!values.year) {
    ...
    if (terms2) {
      if (driverStatus === false) {
        ...
        try {
          ...
          formdata.append(
            "data",
            JSON.stringify({ ...formData, Driver, driver_id: res.data.id })
          );
          const response = await axios.post(`${BASE_URL}/api/cars`, formdata);
          if (response.statusText === "OK") {
            dispatch(setRegisterCarform2({}));
